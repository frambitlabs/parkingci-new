...
class Parking extends Admin_Controller 
{
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('plate', 'Plate No', 'required|alpha_numeric');
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	$parking_code = strtoupper('nm-'.substr(md5(uniqid(mt_rand(), true)), 0, 8));
	...
	public function edit($id = null)
	{
		...
		if($id) {
			$this->form_validation->set_rules('parking_slot', 'Slot', 'required');
			$this->form_validation->set_rules('plate', 'Plate No', 'required|integer');
			$this->form_validation->set_rules('vehicle_cat', 'Category', 'required');
			$this->form_validation->set_rules('vehicle_rate', 'Rate', 'required');
					...
					<div class="main-content">
						...
						<div class="parking-info">
							<table class="table">
								...
								<tr>
									<td>Parking no: '.$parking_data['plate'].' </td>
								</tr>
							</table>
